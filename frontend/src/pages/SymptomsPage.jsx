...
export default function SymptomsPage() {
  const { session, updateLanguage, addInsight } = useSession();
  ...
  const handleAnalyze = async (textOverride) => {
    ...
    try {
      const data = await api.analyzeSymptoms(text, session.language);
      if (data.insights) addInsight(data.insights);
  ...
  return (
    <div className="p-6 max-w-2xl mx-auto pb-24 lg:pb-6" dir={rtl ? "rtl" : "ltr"}>
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          <h1 className="text-xl font-semibold" style={{ color: "var(--color-slate-800)" }}>{t(session.language, "symptoms_title")}</h1>
